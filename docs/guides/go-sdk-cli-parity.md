...
- Admin tenant/quota API: TiDBCloud Mode tenant list/get with quota and quota update.
...
| `admin tenant list/get/set-quota` | TiDBCloud Mode tenant read with quota and TiDB Cloud-only quota update, human/JSON output, local credential resolution | SDK has typed admin tenant and quota set helpers | No CLI-style formatter, region-code server selection helper, or local context/env credential resolver in the SDK. |
