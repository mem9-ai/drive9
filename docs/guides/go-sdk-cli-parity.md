...
- Admin tenant/quota API: TiDB Cloud Native tenant list/get with quota and quota update.
...
| `admin tenant list/get`, `admin quota set` | TiDB Cloud Native tenant read with quota and TiDB Cloud-only quota update, human/JSON output, local credential resolution | SDK has typed admin tenant and quota set helpers | No CLI-style formatter, region-code server selection helper, or local context/env credential resolver in the SDK. |
