...
- **A signal-plane outage stalls online clients by default.** Events stay durable in `fs_events`, but if the meta DB is down long enough that wake-up signals are never written (or the outbox poller stalls), a connected client is simply never woken and new events sit undelivered until the next successful signal or a reconnect; operators who need connected clients to self-heal during such outages can set `DRIVE9_SSE_LIVENESS_POLL_INTERVAL` (default off), which trades a bounded poll against the DBs of tenants with live SSE connections.
