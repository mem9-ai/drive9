...
| --- | --- | --- |
| `coding-agent` | Dependency and build-output trees (`node_modules`, `.venv`, `target`), overlaid only when the repository also ignores them (the gitignore-aware gate; see [git-fast-clone-workspace](./git-fast-clone-workspace.md)). `.git` is local only inside a Git workspace. This is the default FUSE profile. | No automatic pack paths. Nothing is packed unless the user explicitly asks. |
