...
| --- | --- | --- |
| `coding-agent` | `[local]` dependency trees (`node_modules`, `.venv`, overlaid unconditionally) and `[local-gitignore-aware]` build output (`target`, overlaid only when the repository also ignores it; see [git-fast-clone-workspace](./git-fast-clone-workspace.md)). `.git` is local only inside a Git workspace. This is the default FUSE profile. | No automatic pack paths. Nothing is packed unless the user explicitly asks. |
