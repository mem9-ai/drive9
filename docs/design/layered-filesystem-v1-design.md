...
- `.git` and `node_modules` prefer the existing local-only overlay; other build/cache output stays remote-managed unless the profile opts it in.
...
1. `local-only` policy: `.git`, `node_modules`, and (when locally ignored) Rust `target/` continue using the existing local overlay.
