...
The profiling design therefore combines Go pprof with a continuous JSONL sample
stream and a reproducible `perf/mount` harness.
...
The stream is intentionally append-only and line-delimited so it can be tailed,
uploaded from customer environments, or post-processed without a database.
