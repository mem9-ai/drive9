...
| `buffer_drop_oldest` (per tenant; `reason` ∈ `tenant_cap`, `global_cap`, `expired`, `stopped`) | Hard loss (buffer overflow, entry aged past the retry max age = fs_events retention clamped to [1h, 24h], or enqueue after shutdown stop) | ≈ 0; page on non-zero |
