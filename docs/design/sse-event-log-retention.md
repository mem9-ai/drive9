...
- Phase 2: replace fixed-delay catch-up with an **event-driven drain**: when `pollAndSend` returns a full page, pull again immediately; when it returns a short page, disarm. No standing poll, no fixed timer spinning against cross-region RTTs — quiet tenants still generate zero tenant-DB traffic.
...
| `buffer_drop_oldest` (per tenant) | Hard loss (buffer overflow) | ≈ 0; page on non-zero |
