...
5. Buffered entry dropped before its durable insert without overflow: either the entry aged past the retry max age (set from the fs_events retention, clamped to [1h, 24h] — a 168h retention is thus actually reachable by retried events, while memory stays bounded), or it was enqueued after the buffer stopped during shutdown. Both are counted per tenant (`reason="expired"` / `reason="stopped"`) and alerted like mode 2.
...
| `buffer_drop_oldest` (per tenant; `reason` ∈ `tenant_cap`, `global_cap`, `expired`, `stopped`, `shutdown`) | Hard loss (buffer overflow, entry aged past the retry max age = fs_events retention clamped to [1h, 24h], enqueue after shutdown stop, or leftover after the shutdown flush budget) | ≈ 0; page on non-zero |
