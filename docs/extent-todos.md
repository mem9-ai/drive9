...
1. **Other sources of orphan jfs edges?** The server-side directory paths are fixed (`DeleteEmptyDir`, `DeleteDirRecursive`, `RenameDir` all touch the jfs tree, and `RenameDir` now mirrors a missing destination parent instead of skipping the edge move). `DeleteNodesByPrefix` has no callers today; a new path that deletes `file_nodes` rows directly needs the same cleanup (`grep -rn "DELETE FROM file_nodes"`).
2. **A rename over an OPEN extent file reclaims it immediately.** `jfsRenameTx` removes a replaced destination through `jfsUnlinkTx(..., opened: false)`, even when a mount still holds that file open, so the node and its blocks go with the rename and the open handle's later reads/writes fail. Upstream JuiceFS preserves a same-mount handle (`doRename` computes `opened = m.of.IsOpen(dn.Inode)` and keeps the node through the sustained mechanism), and the fork's unlink path already carries that plumbing — only rename does not, and neither the rename meta op nor the FUSE request has a field to carry it. The rename itself stays atomic and complete (no data loss, no corruption); the replaced file's bytes are reclaimed while a stale handle can still ask for them.

   **Fix sketch**: add `Opened`/`sid` to the `rename` op payload, pass `len(fs.openHandles.SnapshotPath(newP)) > 0` from `Dat9FS.extentRename` (resolve the replaced node before the call), mirror the field in the fork's `drive9RenameReq` for VFS-driven renames, and let `jfsRenameTx` route the destination removal through `jfsUnlinkTx`'s opened branch. `docs/design/extent-content-layout.md` §3.3 records the deviation for now.
3. **`extentRuntimePool.retired` is a permanent tombstone.** `forget` (tenant deletion) records the tenant ID and nothing ever clears it, so a control plane that re-provisions the same ID after the deletion is durable keeps getting "tenant has no worker" from the server-side compaction fallback until the process restarts (mounts still compact; the failure is logged, not silent, but an operator has no reason to connect it to a restart). It cannot simply be cleared on a later kick: the tombstone exists precisely to stop a worker that resolved the tenant *before* the deletion from rebuilding a Runtime, and a kick that read "active" can be in flight across the delete.

   **Fix sketch**: key the tombstone to a tenant generation instead of the bare ID (the pool already treats the `*datastore.Store` pointer as a backend generation, and `entryFor` could hand the generation it saw to `withRuntime`, which then refuses only when the tenant was retired *after* that snapshot), or un-retire from the lifecycle event that durably re-creates the tenant.
4. **The extent usage report can still double-report during a shard-ring transition.** `reportExtentQuotaUsage` runs only on the tenant's shard owner and commits the marker with a compare-and-set against the value it peeked, so in steady state exactly one pod reports a delta, and a report that lands while the marker write fails is re-reported. What remains is the ordering: ownership is a hash ring, so a transition can hand the tenant over while a report is in flight, and both pods send their delta to the central counters *before* either commits. The CAS then keeps the marker from regressing (the loser logs `extent_quota_report_raced`), but its delta is never taken back, and central counters are relative increments that nothing recomputes, so that extra amount persists — fail-closed for a soft quota, wrong usage/billing. Fix sketch: a report lease/claim (for example a second counter row holding `claimed_by`/`claimed_at`, with the marker advancing only in the transaction that claims the delta) so the second pod never reports it at all.

   **Not the fix**: reading the marker without the total in the same transaction, or reporting the total instead of the delta — both re-introduce the double count that `ExtentUnreportedUsageBytes` exists to prevent.
5. **Should a directory expose its `extent_ino`?** `jfsMknodTx`/`jfsEnsureParentsTx` already write `extent_ino` on directory rows, but no `content_layout`. Nothing needs it today (`RenameDir` moves the edge, and `jfsEnsureParentsTx` mirrors parents on demand), but a client that had to resolve a directory's jfs inode without a name would need a stat/header field plus a FUSE adoption rule.
...
7. **Do not delete a block before its grace period**: `available_at` exists because a reader on another mount may still hold the pre-compaction slice list.
8. **Do not re-enable JuiceFS chunk-store writeback** (`chunk.Config.Writeback`): with it, `Finish` stages the block locally and the object PUT happens afterwards, so Flush/Fsync would return before the data is remote-durable. The cache dir is still the local read cache.
9. **Locks belong to the metadata service**: `setlk`/`getlk`/`flock` run in one TiDB transaction against `jfs_plock`/`jfs_flock` (node row `FOR UPDATE`), like JuiceFS's own SQL engine. Do not reintroduce a process-local lock table — two mounts of one filesystem must exclude each other.
