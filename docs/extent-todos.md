...
**Landed fix: read from the write buffer instead of draining the inode** [verified, fork `e7a7fe2a`, drive9 `4c3cb5d5` "extent: answer reads from the JuiceFS write buffer"]
...
**Still open on this item**: `crash01` is now corrupt-free but not yet reliably `rc=0`. Its remaining failure is `mptest`'s `--wait all` timeout in `crash02.subtest` line 53: the client that crashes (`--exit 1`) holds the SQLite write lock for the whole duration of its task, and the peers' `UPDATE task` bookkeeping runs into their 10 s busy timeout while it is still going. Measured on the mount's own perf counters, the close-time flush is **not** the cause (`flush count=46 avg=2.1 ms p95=32 ms max=20.6 ms` for a whole `crash01` run; `fsync avg=10.3 ms`), so the bound is simply how long the crashing client's transaction takes: **a ~35 s spill cannot fit a 10 s busy timeout**. Cap-on passes because the same transaction takes ~13 s there. So the last blocker for requirement #1 is the spill itself: bring it from ~38 s to ~10 s. The same test passes on an idle host (0/94, 25–33 s) and its corruption is gone. Two independent facts from this session bound the problem: native JuiceFS 1.4.1 against the same local TiDB + MinIO **also fails** `crash01` with the cap off (300 s timeout, meta ops averaging 5.7 ms, `setlk` in the meta path), and the instance's disk filling to 100 % wedged TiDB and produced spurious corruption — check `df -h /` before trusting any cap-off failure on this host.
...
The same suite passes **20/20 on macFUSE** (which ignores the writeback-cache cap, so there is no real kernel writeback cache), so the failure is writeback-cache-dependent. It is **not a regression from the classic durability fixes** (the 2026-06→09 commits — zombie handles, O_APPEND local fds, close-sync durability; their SHAs changed in the 2026-09 history cleanup, search by subject): before those fixes the suite never reached section `[6]` on CI — earlier commits failed in the release gate (git clone EIO, hardlink, truncate/unlink) and aborted before the SQLite suite ran. The durability fixes are what let the suite run this far for the first time.
