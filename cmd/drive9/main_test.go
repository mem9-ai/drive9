...
func TestDispatchHelpCommandShowsVisualTreeHelp(t *testing.T) {
	...
	for _, want := range []string{
		...
		"drive9 mount status [--json] <mountpoint>",
		"drive9 mount health <mountpoint>",
		"drive9 mount ensure [--reset] [--restart] <mountpoint>",
		"drive9 mount systemd-unit [--install] [--name name] [mount flags] <mountpoint>",
