	...
	return []visualHelpCommand{
		...
		},
		{
			...
			Flags: []visualHelpFlag{
				...
				{Name: "--max-media-llm-files N", Desc: "set-quota: max media LLM extract files; non-negative; 0 means unlimited; omit to keep current; requires media extract config"},
				{Name: "--max-video-llm-files N", Desc: "set-quota: max video LLM extract files; non-negative; 0 means unlimited; omit to keep current; requires video extract config"},
				{Name: "--tidbcloud-spending-limit N", Desc: "tenant create/set-quota TiDB Cloud Cluster Spending Limit"},
				{Name: "--json", Desc: "output result as JSON"},
			},
			Examples: []visualHelpExample{
				{Command: "drive9 admin tenant create --region-code aws-ap-southeast-1 --tidbcloud-public-key <public-key> --tidbcloud-private-key <private-key>", Desc: "create a TiDBCloud Mode tenant"},
		...
		},
		{
			...
			Examples: []visualHelpExample{
				...
				{Command: "drive9 fs archive :/proj ./proj.tar.gz --profile coding-agent", Desc: "download tree, skip node_modules/target"},
		...
		},
		{
			...
			Sections: []visualHelpSection{
				...
				{Title: "Supervision", Flags: []visualHelpFlag{
					...
					{Name: "--supervised", Desc: "internal: worker managed by the supervisor; set by the supervisor itself"},
				}},
				...
				{Title: "Overlay profiles", Flags: []visualHelpFlag{
					{Name: "--profile NAME", Desc: "mount profile: coding-agent, portable, none, extent, interactive, or a custom profile file"},
					...
					{Name: "--local-only-gitignore-aware PATTERN", Desc: "overlay matching paths only when the repository also ignores them; repeatable; adds a [local-gitignore-aware] rule"},
					...
					{Name: "--append-log PATTERN", Desc: "use append-log sync optimization for matching remote-persistent files; repeatable; env DRIVE9_MOUNT_APPEND_LOG_PATTERNS is newline-delimited"},
				...
				{Title: "Layers and writes", Flags: []visualHelpFlag{
					...
					{Name: "--write-cache-size-mb MB", Desc: "shadow cache byte quota in MB; 0 disables; writes exceeding it return ENOSPC"},
					...
					{Name: "--writeback-batch-max-files N", Desc: "maximum files in one writeback batch when enabled"},
					{Name: "--writeback-batch-max-bytes BYTES", Desc: "maximum bytes in one writeback batch when enabled"},
				}},
			...
			Examples: []visualHelpExample{
				...
				{Command: "drive9 mount --extent '*.db' :/data ./mnt", Desc: "store matching files as content_layout=extent"},
