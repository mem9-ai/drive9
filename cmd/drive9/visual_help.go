	...
	return []visualHelpCommand{
		...
		},
		{
			...
			Flags: []visualHelpFlag{
				...
				{Name: "--max-media-llm-files N", Desc: "set-quota: max media LLM extract files; must be non-negative; requires media extract config"},
				{Name: "--max-video-llm-files N", Desc: "set-quota: max video LLM extract files; must be non-negative; requires video extract config"},
