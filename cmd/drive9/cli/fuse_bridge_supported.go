...
func mountFuseImpl(opts *mountFuseOptions) error {
	...
	return drive9fuse.Mount(&drive9fuse.MountOptions{
		...
		LocalGitignoreAwarePatterns: opts.LocalGitignoreAwarePatterns,
		RemoteOnlyPatterns:          opts.RemoteOnlyPatterns,
		// The CLI resolves the effective profile (builtin or custom) into the
		// pattern lists above, so FUSE must not add builtin defaults again by
		// profile name.
		DisableBuiltinOverlayDefaults: true,
		...
		DisableDeferredUnlink:         opts.DisableDeferredUnlink,
		WriteBackBatchWindow:          opts.WriteBackBatchWindow,
		WriteBackBatchMaxFiles:        opts.WriteBackBatchMaxFiles,
		WriteBackBatchMaxBytes:        opts.WriteBackBatchMaxBytes,
