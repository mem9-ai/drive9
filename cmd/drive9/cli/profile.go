...
//
// VCS metadata stays here: `git clone --fast` writes the working `.git` before
// the workspace is registered, so a workspace-scoped rule would miss it. Other
// build/cache output stays remote-persistent.
