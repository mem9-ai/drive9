...
DEFAULT_LTP_FS_EXCLUDE = (
    ...
    "squashfs01",
    # fs_racer — designed as sleep(5) then cleanup, but workers dd up to ~2GB
    # files while other workers rm -rf them. killall of the .sh wrappers does
    # not reap child dd; rm -rf of those in-flight FUSE writes does not finish
    # in the module budget (observed: 23+ min hang until 1800s timeout).
    "fs_racer",
)
...
DEFAULT_LTP_SYSCALL_EXCLUDE = (
    ...
    "open09", "open15",
    # open04 — loops open(O_CREAT) until EMFILE, bounded by RLIMIT_NOFILE.
    # On hosts with ulimit -n in the hundreds of thousands (e.g. Orb Arch
    # 1048576) this is O(n) FUSE creates and dominates the shard (~8 min);
    # GitHub ubuntu-latest (n≈1024) finishes quickly. Skip rather than
    # depend on the host nofile limit.
    "open04",
)
...
def _ensure_ltp_fs_scenario(source_dir: Path, install_dir: Path) -> int:
    ...
    destination.write_text(
        ...
        "# (read_all_*, proc01, fs_racer). lftest01 is retained (JuiceFS\n"
        "# rm_list.sh bug stripped it; we do not replicate that bug).\n"
        + "\n".join(lines)
        + "\n",
        encoding="utf-8",
    )
