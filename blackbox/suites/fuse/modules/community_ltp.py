...
class CommunityLTPFS(BaseModule):
    ...
    def run(self, ctx: Context) -> dict[str, Any]:
        ...
        handle = ctx.target.mount("community_ltp_fs", remote, durability="write-sync", extra=["--allow-other"])
        try:
...
class CommunityLTPSyscalls(CommunityLTPFS):
    ...
    def run(self, ctx: Context) -> dict[str, Any]:
        ...
        remote = ctx.target.remote_root(self.id)
        ctx.target.mkdir_remote(remote)
        handle = ctx.target.mount("community_ltp_syscalls", remote, durability="write-sync", extra=["--allow-other"])
        try:
            work = handle.mountpoint / "ltp-work"
            work.mkdir()
            result = ctx.target.run_cmd(
