...
- `custom.*`: reproducible case-scenario benchmarks. These are
  ...
  suites/
    fuse/
      ...
      modules/
        ...
        custom_*.py        case-scenario benchmark modules
```
...
- `custom`: opt-in case-scenario modules. These are intentionally not
...
make blackbox BLACKBOX_SELECTOR=group:custom
...
## case1 Performance Module

`custom.case1` codifies the Case1 sandbox workload:
...
- one measurement round by default, controlled by `BLACKBOX_RUNS` or
  `BLACKBOX_CASE1_PERF_RUNS`; set it to `3` when a three-round average is
  required;
...
This module is never executed accidentally. It is registered in the `custom`
group, but `ensure_dependencies` skips it unless explicitly enabled:

```bash
BLACKBOX_CASE1_PERF_ENABLE=1 \
BLACKBOX_SERVER_MODE=existing \
DRIVE9_BASE=http://drive9.pingkai.cn \
make blackbox BLACKBOX_SELECTOR=module:custom.case1
```
...
BLACKBOX_CASE1_PERF_ENABLE=1 \
BLACKBOX_RUNS=3 \
BLACKBOX_CASE1_PERF_SCALES=S,M,L \
BLACKBOX_CASE1_PERF_DATASET_TIMEOUTS=S=300,M=600,L=120 \
BLACKBOX_CASE1_PERF_NAMESPACE_CMD_TIMEOUTS=S=180,M=30,L=30 \
...
```bash
BLACKBOX_CASE1_PERF_LAYOUTS=single,tree
BLACKBOX_CASE1_PERF_PROFILE=coding-agent
BLACKBOX_CASE1_PERF_DURABILITY=auto
BLACKBOX_CASE1_PERF_RUNS=3
BLACKBOX_CASE1_PERF_STAT_SAMPLES=1000
BLACKBOX_CASE1_PERF_DATASET_TIMEOUT_S=300
BLACKBOX_CASE1_PERF_DATASET_TIMEOUTS=S=300,M=600,L=120
BLACKBOX_CASE1_PERF_NAMESPACE_CMD_TIMEOUT_S=300
BLACKBOX_CASE1_PERF_NAMESPACE_CMD_TIMEOUTS=S=180,M=30,L=30
BLACKBOX_CASE1_PERF_SMALL_SIZES=1024,4096,20480,102400,1048576
BLACKBOX_CASE1_PERF_SMALL_OPS=50
BLACKBOX_CASE1_PERF_SMALL_CONCURRENCY=1,4,16,64
BLACKBOX_CASE1_PERF_FLUSH_SIZES=1024,4096,20480,102400,1048576
BLACKBOX_CASE1_PERF_FLUSH_OPS=30
BLACKBOX_CASE1_PERF_FLUSH_CONCURRENCY=1,4,16,64
BLACKBOX_CASE1_PERF_VISIBILITY_SAMPLES=100
BLACKBOX_CASE1_PERF_MOUNT_COUNTS=1,2,5,10
BLACKBOX_CASE1_PERF_REMOTE_ROOT=/some/reusable/remote/root
BLACKBOX_CASE1_PERF_REUSE_DATASETS=1
BLACKBOX_CASE1_PERF_RAW=1
FUSE_POST_UMOUNT_WAIT_S=3
```
...
```bash
BLACKBOX_CASE1_PERF_NAMESPACE=1
BLACKBOX_CASE1_PERF_SMALL_FILE=1
BLACKBOX_CASE1_PERF_FLUSH=1
BLACKBOX_CASE1_PERF_PERSISTENCE=1
BLACKBOX_CASE1_PERF_MULTI_MOUNT=1
BLACKBOX_CASE1_PERF_SOAK=0
```

Outputs are written under the normal run directory:

```text
blackbox/results/fuse/<session>/artifacts/custom.case1/
  environment.json
  manifest.json
  raw_results/*.jsonl
  summary/summary.csv
