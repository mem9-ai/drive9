...
| `smoke-all.sh` | Runs API + CLI + journal + FUSE + POSIX permission smoke scripts in sequence with aggregated pass/fail; set `RUN_GIT_WORKSPACE_SMOKE=1` to include Git workspace coverage and `RUN_PORTABLE_PACK_E2E=1` to include portable pack/unpack coverage |
