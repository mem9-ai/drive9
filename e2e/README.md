...
| `fuse-s3-express-append-log.sh` | Manual-only hosted FUSE SQLite WAL workload: configurable `synchronous=FULL` transactions, same-mount reopen, fresh remount, integrity/fingerprint checks, and append-log perf artifacts. Trusted MicroVM cases verify `32B generation PUT -> fresh local shadow -> first-frame tail append` and, over 2,000 commits, that second-checkpoint WAL reads stay local. Requires a real S3 Express Directory Bucket; intentionally excluded from local-e2e and smoke-all. |
