...
| `fuse-smoke-test.sh` | FUSE mount lifecycle, file/dir/symlink/hardlink/rename/stat semantics, cross-channel consistency, `mount drain`/native `sync -f` drain checks, mounted 10KiB→8MiB→10KiB tier-transition parity, read-only and error-path checks; with `RUN_SYNCHRONOUS_PROMOTION_E2E=1` against a compatible external server it also exercises feature-off `EXDEV`, open/target rejection, response-loss + mount-restart recovery, and real two-mount synchronous `dist/.site-next` → `site` promotion through direct `rename(2)` calls (never `mv` copy fallback) |
...
| `tasks-smoke-test.sh` | `fs tasks` wire contract for `GET /v1/fs/{path}?tasks`: `X-Dat9-Tasks: 1` marker, `{path, tasks[]}` field names, `tasks` always an array (never null), directory 400, and unknown-query-key rejection. Cross-repo drift check while the handler lives in the server repository. Off in default `smoke-all.sh`; set `RUN_TASKS_SMOKE=1` |
...
| `s3-express-append-log-smoke-test.sh` | Manual-only real Directory Bucket HTTP append-log coverage: create/append, conflict, bounded range/no redirect, 5GB rejection, full-body rewrite, and 10,000-part rebase+retry. Requires a server with S3 Express append-log enabled; intentionally excluded from local-e2e and smoke-all. |
| `fuse-s3-express-append-log.sh` | Manual-only hosted FUSE SQLite WAL workload: configurable `synchronous=FULL` transactions, same-mount reopen, fresh remount, integrity/fingerprint checks, and append-log perf artifacts. Trusted MicroVM cases `issue-875-append-log-trace` and `issue-875-auto-reset-header-first` verify trace evidence and `32B generation PUT -> fresh local shadow -> first-frame tail append`; the 2,000-commit contrast case asserts second-checkpoint WAL reads stay local. They require a real S3 Express Directory Bucket and trusted dynamic-case transport, so are intentionally excluded from local-e2e and smoke-all. |
...
# 1000 sequential WAL FULL commits, reopen, remount, remote snapshot.
...
To run FUSE suites with `coding-agent-extent` (all files `content_layout=extent`):
...
- FUSE SQLite commit-sequence knobs are `FUSE_SQLITE_COMMITS` (default 1000), `FUSE_SQLITE_WAL_AUTOCHECKPOINT` (default 100), `FUSE_SQLITE_COMMIT_TIMEOUT_S`, `FUSE_SQLITE_COMMIT_DURABILITY` (default `fsync`), `FUSE_SQLITE_COMMIT_KEEP_ARTIFACTS`, and `FUSE_PROFILE`. This is the generic (non-S3-Express) 1000-commit WAL FULL detector.
...
- FUSE release-gate knobs are `FUSE_STRICT_PREREQS`, `RUN_FUSE_GIT_CLONE`, `FUSE_GIT_CLONE_URL`, `FUSE_GIT_CLONE_TIMEOUT_S`, `RUN_FUSE_UMOUNT_DURABLE`, `FUSE_UMOUNT_TIMEOUT`, `RUN_FUSE_LOG_AUDIT`, `RUN_SYNCHRONOUS_PROMOTION_E2E`, `RUN_FUSE_ALL_WORKLOADS`, `RUN_FUSE_SQLITE_CORRECTNESS`, `RUN_FUSE_SQLITE_COMMIT_SEQUENCE`, `RUN_FUSE_CONCURRENCY_STRESS`, `RUN_FUSE_POSIX_FSX`, `RUN_FUSE_PERFORMANCE_BASELINE`, and the FUSE correctness/SQLite/concurrency/POSIX/fsx/performance workload knobs. `RUN_SYNCHRONOUS_PROMOTION_E2E=1` adds the real two-mount `dist/.site-next` promotion and response-loss restart cases; it is deliberately off in this repository's ordinary gate because the server implementation is owned by `tidbcloud/fs`. The cross-repository server gate enables it while pinning both repositories, and starts the server with `DRIVE9_ENABLE_SYNCHRONOUS_PROMOTION=1`. Set `RUN_FUSE_ALL_WORKLOADS=1` to default concurrency stress, POSIX/fsx, and performance baseline to enabled in one release-gate command; explicit per-workload env vars still take precedence. `local-e2e.yml` intentionally overrides `RUN_FUSE_CONCURRENCY_STRESS=0` and `RUN_FUSE_POSIX_FSX=0` for its release-gate step, then runs `fuse-concurrency-stress.sh`, `fuse-posix-fsx-gate.sh`, and `fuse-sqlite-commit-sequence.sh` separately after metrics artifact/archive steps.
