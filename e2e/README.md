...
| `tasks-smoke-test.sh` | `fs tasks` wire contract for `GET /v1/fs/{path}?tasks`: `X-Dat9-Tasks: 1` marker, `{path, tasks[]}` field names, `[]` (never null) empty result, directory 400, and unknown-query-key rejection. Cross-repo drift check while the handler lives in the server repository. Off in default `smoke-all.sh`; set `RUN_TASKS_SMOKE=1` |
