...
| `s3-express-tenant-delete-config-transition.md` | Manual-only server runbook for a disposable tenant across target-A, mismatched target-B, and restored target-A deployments. It proves tenant deletion retries without finalization or target-B access while configuration mismatches, then cleans target A after restoration. |
