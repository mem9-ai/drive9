#!/usr/bin/env bash
# Hosted COS --auth=server coverage: prefix isolation, fail-closed mint,
# read-only STS, drive9↔COS copy, FUSE mount, and in-place STS refresh.
#
# Manual-only. Uses a dedicated Tencent test bucket only — never prod COS.
# List isolation is checked with tccli against minted federation tokens.
#
...
#   export DRIVE9_E2E_OBJECT_BUCKET="your-bucket-appid"
#   export DRIVE9_E2E_OBJECT_ACCESS_KEY_ID="..."
#   export DRIVE9_E2E_OBJECT_SECRET_ACCESS_KEY="..."
#   export DRIVE9_E2E_OBJECT_REGION=ap-beijing
#   export DRIVE9_E2E_OBJECT_ACCOUNT_ID=125xxxxxxxx
...
if [ "${#missing_env[@]}" -gt 0 ]; then
  printf 'SKIP: object-auth cos hosted coverage requires: %s\n' "${missing_env[*]}"
  exit 0
fi
...
SCHEME="${DRIVE9_E2E_OBJECT_SCHEME:-cos}"
BUCKET="$DRIVE9_E2E_OBJECT_BUCKET"
REGION="${DRIVE9_E2E_OBJECT_REGION:-ap-beijing}"
...
command -v tccli >/dev/null || die "tccli is required for COS STS isolation checks"
...
cos_with_session() {
  ...
  env -u AWS_PROFILE PYTHONWARNINGS=ignore \
    tccli cos "$@" --bucket "$BUCKET" --region "$region" \
    --secretId "$ak" --secretKey "$sk" --token "$tok"
}
...
list_keys() {
  ...
  out="$(mktemp)"
  set +e
  cos_with_session "$creds" list --prefix "$prefix" >"$out" 2>"$out.err"
  set -e
  if grep -Eqi 'AccessDenied|Access Denied|not authorized' "$out" "$out.err"; then
    echo ACCESS_DENIED
    rm -f "$out" "$out.err"
    return 0
  fi
  if grep -Eqi 'Error:|Invalid' "$out" "$out.err"; then
    echo "cos list failed prefix=$prefix $(cat "$out.err" "$out")" >&2
    rm -f "$out" "$out.err"
    return 1
  fi
  awk '/STANDARD|STORAGE/ {print $NF}' "$out"
  rm -f "$out" "$out.err"
}
...
echo "=== drive9 object-auth COS hosted coverage ==="
...
add_args=(object-backend add --scheme "$SCHEME" --bucket "$BUCKET" --json "${ADMIN_FLAGS[@]}"
  ...
  --region "$REGION" --name "cos-cov-${TS}" --credential-kind static)
...
got_body="$(mktemp)"
set +e
cos_with_session "$mint_ro" download --cos_key "$(native_key "$NS_A" hello.txt)" --local_path "$got_body" >"$got_body.log" 2>&1
ro_get_rc=$?
set -e
...
ro_put="$(mktemp)"
set +e
cos_with_session "$mint_ro" upload --local_path "$FILE_A" --cos_key "$(native_key "$NS_A" from-ro.txt)" >"$ro_put" 2>"$ro_put.err"
ro_rc=$?
set -e
if grep -Eqi 'AccessDenied|Access Denied' "$ro_put" "$ro_put.err"; then
  ro_rc=1
fi
check_cmd "read-only session cannot PutObject" test "$ro_rc" -ne 0

echo "--- drive9 <-> COS copy ---"
...
check_cmd "COS -> drive9 copy" contains "$d9_got" "alpha-"
...
check_cmd "drive9 -> COS copy" contains "$rt_got" "alpha-"
...
echo "=== object-auth COS hosted coverage done PASS=$PASS FAIL=$FAIL TOTAL=$TOTAL ==="
if [ "$FAIL" -gt 0 ]; then
  echo "mount log:" >&2
  cat "$MOUNT_LOG" >&2 || true
  exit 1
