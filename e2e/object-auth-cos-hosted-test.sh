...
got_body="$(mktemp)"
set +e
cos_with_session "$mint_ro" download --cos_key "${NS_A}/hello.txt" --local_path "$got_body" >"$got_body.log" 2>&1
ro_get_rc=$?
set -e
...
ro_put="$(mktemp)"
set +e
cos_with_session "$mint_ro" upload --local_path "$FILE_A" --cos_key "${NS_A}/from-ro.txt" >"$ro_put" 2>"$ro_put.err"
ro_rc=$?
set -e
