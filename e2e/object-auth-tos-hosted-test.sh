#!/usr/bin/env bash
# Hosted TOS --auth=server coverage: prefix isolation, fail-closed mint,
# read-only STS, drive9↔TOS copy, FUSE mount, and in-place STS refresh.
#
# Manual-only. Uses a dedicated Volcengine test bucket only.
# List isolation is checked with tosutil against minted AssumeRole tokens.
#
...
#   export DRIVE9_E2E_OBJECT_ROLE_ARN="trn:iam::ACCOUNT:role/your-role"
#   export DRIVE9_E2E_OBJECT_REGION=cn-beijing
...
if [ "${#missing_env[@]}" -gt 0 ]; then
  printf 'SKIP: object-auth tos hosted coverage requires: %s\n' "${missing_env[*]}"
  exit 0
fi
...
SCHEME="${DRIVE9_E2E_OBJECT_SCHEME:-tos}"
BUCKET="$DRIVE9_E2E_OBJECT_BUCKET"
REGION="${DRIVE9_E2E_OBJECT_REGION:-cn-beijing}"
...
command -v tosutil >/dev/null || die "tosutil is required for TOS STS isolation checks"
if [ -z "$ROLE_ARN" ]; then
  die "DRIVE9_E2E_OBJECT_ROLE_ARN is required for TOS AssumeRole mint"
fi
...
tos_conf_from_creds() {
  local creds="$1" conf="$2"
  python3 - "$creds" "$conf" "$REGION" <<'PY'
import json, subprocess, sys
from pathlib import Path
creds, conf, region = sys.argv[1], sys.argv[2], sys.argv[3]
d=json.load(open(creds))
Path(conf).write_text("")
ep=d.get("endpoint") or ("https://tos-s3-"+region+".volces.com")
# tosutil wants host without requiring virtual-host; native endpoint is fine for isolation checks
if "tos-s3-" in ep:
    ep="tos-"+region+".volces.com"
ep=ep.replace("https://","").replace("http://","")
subprocess.check_call(["tosutil","config",
  "-i="+d["access_key_id"], "-k="+d["secret_access_key"], "-t="+d["session_token"],
  "-e="+ep, "-re="+region, "-conf="+conf], stdout=subprocess.DEVNULL)
PY
}
...
list_keys() {
  ...
  local conf out
  conf="$(mktemp)"
  tos_conf_from_creds "$creds" "$conf"
  out="$(mktemp)"
  set +e
  tosutil ls "tos://${BUCKET}/${prefix}" -conf="$conf" >"$out" 2>"$out.err"
  set -e
  rm -f "$conf"
  if grep -Eqi 'AccessDenied|Access Denied|Forbidden|0003-00000008' "$out" "$out.err"; then
    echo ACCESS_DENIED
    rm -f "$out" "$out.err"
    return 0
  fi
  if grep -Eqi '^Error:' "$out" "$out.err"; then
    echo "tos list failed prefix=$prefix $(cat "$out.err" "$out")" >&2
    rm -f "$out" "$out.err"
    return 1
  fi
  awk '/tos:\/\// {print $1}' "$out" | sed -e "s#^tos://${BUCKET}/##"
  rm -f "$out" "$out.err"
}
...
echo "=== drive9 object-auth TOS hosted coverage ==="
...
add_args=(object-backend add --scheme "$SCHEME" --bucket "$BUCKET" --json "${ADMIN_FLAGS[@]}"
  ...
  --region "$REGION" --name "tos-cov-${TS}" --credential-kind role --role-arn "$ROLE_ARN")
...
check_cmd "TOS -> drive9 copy" contains "$d9_got" "alpha-"
...
check_cmd "drive9 -> TOS copy" contains "$rt_got" "alpha-"
...
echo "=== object-auth TOS hosted coverage done PASS=$PASS FAIL=$FAIL TOTAL=$TOTAL ==="
if [ "$FAIL" -gt 0 ]; then
  echo "mount log:" >&2
  cat "$MOUNT_LOG" >&2 || true
  exit 1
