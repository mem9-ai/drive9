...
func writePerfWorkloadDesign(b *strings.Builder, scenarios []PerfScenario) {
	...
	fmt.Fprintln(b)
	if modes["upload_warm"] && modes["download_file"] && modes["download_null"] {
		fmt.Fprintln(b, "This phase is the broad scan. It runs 50 MiB, 100 MiB, and 1 GiB files, concurrency 1, 2, 4, 8, 16, and 32, with `upload_warm`, `download_file`, and `download_null`. The important comparison is `download_file` versus `download_null`: if `download_null` keeps scaling while `download_file` plateaus, the first bottleneck is on the local output-file path rather than Drive9 read or network receive.")
	} else if modes["download_file"] && modes["download_fsync"] {
		fmt.Fprintln(b, "This phase is targeted disambiguation. It compares ordinary local-file download with local-file download plus explicit fsync. If `download_fsync` is materially worse than `download_file`, durable final flush is a likely bottleneck. If they are similar, the bottleneck is the broader local output/writeback path, not fsync alone.")
	} else {
