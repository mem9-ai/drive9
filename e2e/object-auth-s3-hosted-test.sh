...
check_cmd "read-only session can GetObject" \
  aws_with_session "$mint_ro" s3api get-object --bucket "$BUCKET" --key "$(native_key "$NS_A" hello.txt)" "$got_body"
check_cmd "read-only GetObject body matches" contains "$(cat "$got_body")" "alpha-"
ro_put="$(mktemp)"
set +e
aws_with_session "$mint_ro" s3api put-object --bucket "$BUCKET" --key "$(native_key "$NS_A" from-ro.txt)" --body "$FILE_A" >"$ro_put" 2>"$ro_put.err"
ro_rc=$?
set -e
