...
const (
	...
	// eventRetryMaxAge is how long an entry may sit in the buffer before it is
	// dropped (counted as hard loss). At the 1h default retention an older
	// entry would be pruned right after landing anyway.
	eventRetryMaxAge = 1 * time.Hour
	// eventRetryShutdownFlushTimeout bounds the final best-effort flush at
	// server shutdown.
	eventRetryShutdownFlushTimeout = 2 * time.Second
)
...
// insertNotify (Server.insertTenantNotify in production).
func newEventRetryBuffer(insertNotify func(tenantID string, workMask int)) *eventRetryBuffer {
...
// stop cancels the flush goroutine, waits for it to exit, then attempts one
// final best-effort flush with a short timeout so buffered entries get a last
// chance to land before the process exits. All entries are made due for this
// flush regardless of their backoff slot.
...
// Every eviction is counted per tenant as hard loss.
...
// older than eventRetryMaxAge (counted as hard loss). Entries that fail (or
// have no store yet) are requeued with per-entry exponential backoff.
func (b *eventRetryBuffer) flushDue(ctx context.Context) {
	...
	for _, e := range b.entries {
		switch {
		case now.Sub(e.enqueuedAt) > eventRetryMaxAge:
...
// A nil store or an insert error returns false so the caller requeues.
func (b *eventRetryBuffer) flushEntry(ctx context.Context, e *eventRetryEntry) bool {
	...
	seq, err := inserter.InsertFSEvent(ctx, e.path, e.op, e.actor, e.ts)
	if err != nil {
...
func (b *eventRetryBuffer) requeue(e *eventRetryEntry, now time.Time) {
	...
	b.mu.Lock()
	if now.Sub(e.enqueuedAt) > eventRetryMaxAge {
		b.mu.Unlock()
		metrics.RecordSSEEventRetryDropped(e.tenantID, e.orgID, "expired")
		return
	}
