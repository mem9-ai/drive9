...
func TestSemanticWorkerKeepsBorrowedTenantBackendAliveDuringInvalidate(t *testing.T) {
	...
	// The write above happened before the server was created, so no outbox
	// row was written and no kick was sent. Kick manually so the worker picks
	// up the task enqueued by the pre-server write.
