...
func (c *Client) ensureExtentRuntime(ctx context.Context) (*extent.Runtime, error) {
	c.extentMu.Lock()
	defer c.extentMu.Unlock()
	if c.extentRT != nil {
		return c.extentRT, nil
	}
	// The runtime outlives the request that happened to build it, but the
	// credential and storage handshake must still observe the caller's
	// deadline: a cancelled request must not sit in a data-credential call.
...
func (c *Client) openExtentStream(ctx context.Context, ino, length uint64, offset, limit int64) (io.ReadCloser, error) {
	rt, err := c.ensureExtentRuntime(ctx)
	if err != nil {
		return nil, err
	}
	off := uint64(0)
	...
	return rt.OpenRead(ctx, ino, length, off, lim)
}
