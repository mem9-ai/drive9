...
type AdminObjectBackendUpdateRequest struct {
	PublicKey       string  `json:"public_key"`
	PrivateKey      string  `json:"private_key"`
	Name            *string `json:"name,omitempty"`
...
func (c *Client) AdminGetObjectBackend(ctx context.Context, id, publicKey, privateKey string) (*AdminObjectBackend, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, c.baseURL+"/v1/admin/object-backends/"+url.PathEscape(id), nil)
	if err != nil {
		return nil, err
	}
	setQuotaHeaders(req, publicKey, privateKey)
...
func (c *Client) AdminUpdateObjectBackend(ctx context.Context, id string, in AdminObjectBackendUpdateRequest) (*AdminObjectBackend, error) {
	raw, err := json.Marshal(in)
	if err != nil {
		return nil, err
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPatch, c.baseURL+"/v1/admin/object-backends/"+url.PathEscape(id), bytes.NewReader(raw))
	if err != nil {
		return nil, err
	}
	req.Header.Set("Content-Type", "application/json")
