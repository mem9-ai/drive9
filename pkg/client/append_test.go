...
func TestAppendStreamFallsBackToRewriteWhenS3NotConfigured(t *testing.T) {
	var appendInitiateCalled bool
	var putBody []byte
	var gotExpectedRevision string
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodHead && r.URL.Path == "/v1/fs/db-only.txt":
			w.Header().Set("Content-Length", "5")
			w.Header().Set("X-Dat9-IsDir", "false")
			w.Header().Set("X-Dat9-Revision", "11")
			w.WriteHeader(http.StatusOK)
		case r.Method == http.MethodPost && r.URL.Path == "/v1/fs/db-only.txt" && r.URL.Query().Has("append"):
			appendInitiateCalled = true
			w.WriteHeader(http.StatusBadRequest)
			_ = json.NewEncoder(w).Encode(map[string]string{"error": "S3 not configured"})
		case r.Method == http.MethodGet && r.URL.Path == "/v1/fs/db-only.txt":
			_, _ = io.WriteString(w, "hello")
		case r.Method == http.MethodPut && r.URL.Path == "/v1/fs/db-only.txt":
			gotExpectedRevision = r.Header.Get("X-Dat9-Expected-Revision")
			body, err := io.ReadAll(r.Body)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	if gotExpectedRevision != "11" {
		t.Fatalf("expected revision header = %q, want %q", gotExpectedRevision, "11")
	}
	if got := string(putBody); got != "hello world" {
		t.Fatalf("rewritten body = %q, want %q", got, "hello world")
	}
...
func TestAppendStreamRejectsOverlappingAppendPartWithoutReadURL(t *testing.T) {
	...
	c := New(srv.URL, "")
	err := c.AppendStream(context.Background(), "/large.bin", strings.NewReader("KLMNOP"), 6, nil)
	if err == nil || !strings.Contains(err.Error(), "missing read_url") {
		t.Fatalf("AppendStream error = %v, want missing read_url", err)
	}
	if uploadCalled {
		t.Fatal("upload endpoint should not be called")
	}
	if completeCalled {
