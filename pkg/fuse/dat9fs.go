...
type Dat9FS struct {
	...
	// caches); JuiceFS keeps its chunk cache in the jfs/ subdirectory, so the
	// extent data plane never depends on an explicit --cache-dir.
	...
	// design recommends). Entries expire after extentMissTTL and are cleared
	// whenever this mount creates or unlinks an extent path.
	extentMissesMu sync.Mutex
	extentMisses   map[string]time.Time
...
func NewDat9FS(c *client.Client, opts *MountOptions) *Dat9FS {
	fs := &Dat9FS{
		...
		localPolicy:       NewLocalPolicy(opts.Profile, opts.LocalOnlyPatterns, opts.LocalGitignoreAwarePatterns, opts.RemoteOnlyPatterns, !opts.DisableBuiltinOverlayDefaults),
		...
		directoryPrefetch: newSiblingDirectoryPrefetch(),
	}
	return fs
}
...
func (fs *Dat9FS) applyRemoteTruncate(ctx context.Context, entry *InodeEntry, ino uint64, pid uint32, newSize int64) gofuse.Status {
	...
	if newSize == 0 {
		...
		if fs.canStagePathTruncateToZero(entry) {
			...
			_ = fs.adoptSingleCallerPathTruncate(entry.Path, pid)
			return gofuse.OK
		}
	}
...
func ensureStagedSnapshotLineageLocked(fh *FileHandle) (snapshotID, parentSnapshotID string) {
	if fh == nil {
		return "", ""
	}
...
func clearStagedSnapshotLineageLocked(fh *FileHandle) {
	if fh == nil {
		return
	}
...
// bindCommitEntryToPreparedPendingMeta binds the exact metadata returned by
// PrepareRename. Reading the new path again here could race with a newer Put
// and pair this rename's shadow generation with another snapshot's lineage.
func (fs *Dat9FS) bindCommitEntryToPreparedPendingMeta(entry *CommitEntry, meta *WriteBackMeta) {
	if fs == nil || entry == nil || meta == nil {
		return
	}
	entry.PayloadBaseRev = meta.BaseRev
	entry.PayloadBaseRevSet = true
	entry.PendingIndexGen = meta.Generation
	entry.SnapshotID = meta.SnapshotID
	entry.ParentSnapshotID = meta.ParentSnapshotID
	entry.liveAncestors = processLocalMetaAncestors(meta)
	entry.liveLineageProof = entry.SnapshotID != "" && meta.lineageTrusted
	if watermark := fs.latestCommittedRevision(entry.Path); watermark > meta.BaseRev && meta.BaseRev >= 0 {
		entry.DurableWatermarkRev = watermark
		entry.DisableAutoResolveLWW = true
	}
}
...
func processLocalMetaAncestors(meta *WriteBackMeta) []string {
	if meta == nil {
		return nil
	}
	if len(meta.liveAncestors) > 0 {
...
func (fs *Dat9FS) fillEntryOut(entry *InodeEntry, out *gofuse.EntryOut) {
	...
	// Timeout 0 forced every getattr through VFS.GetAttr HTTP. The mount's own
	// TTLs stay an upper bound (see clampExtentTTL): the create glob also
	// matches classic files, and the extent timeout must not silently lengthen
	// what the user configured for them.
	if entry.ExtentIno != 0 || (!entry.IsDir && fs.shouldUseExtentPath(entry.Path)) {
		ttl := extentAttrTimeout(entry)
		entryTTL = clampExtentTTL(ttl, entryTTL)
		attrTTL = clampExtentTTL(ttl, attrTTL)
	}
...
func (fs *Dat9FS) remoteDirectoryHasChildren(ctx context.Context, dirPath string) (bool, uint64, error) {
	...
	// child another actor deleted would keep this directory looking occupied
	// until the entry expired and block a legitimate Rmdir. The question this
	// function answers is whether the remote directory is empty, and the
	// response is its direct evidence (tombstones above already filter this
	// mount's own in-flight unlinks). A child committed during the RTT is
	// already durable on the server, so the server enforces it regardless of
	// what this pre-check concludes.
	fs.mountViewMu.RUnlock()
...
func (fs *Dat9FS) hardlinkRemoteWithTransientRecovery(ctx context.Context, srcP, dstP string) error {
	if fs.layerEnabled() {
		return fmt.Errorf("hardlink is not supported in fs layer mounts")
	}
...
func (fs *Dat9FS) mkdirRemoteWithTransientRetry(cancel <-chan struct{}, localPath string, mode uint32) error {
	...
	ctx, cf := fuseCtx(cancel)
	if fs.layerEnabled() {
		err := fs.upsertLayerMkdir(ctx, localPath, mode)
		cf()
		return err
	}
	mutationStart := fs.perfStart()
...
func (fs *Dat9FS) renameRemoteWithTransientRetry(ctx context.Context, oldP, newP string) error {
	if fs.layerEnabled() {
		return fs.upsertLayerRename(ctx, oldP, newP)
	}
...
func (fs *Dat9FS) OnUnmount() {
	if fs != nil && fs.directoryPrefetch != nil {
...
func (fs *Dat9FS) GetAttr(cancel <-chan struct{}, input *gofuse.GetAttrIn, out *gofuse.AttrOut) (status gofuse.Status) {
	...
	if !entry.IsDir && fs.extentRefreshFromVFS(input.NodeId, entry.Path, input.Fh()) {
		...
		// The only sanctioned size fabrication in this layer, for an open
		// sqlite WAL index; see sqliteWALIndexMinSize.
...
func (fs *Dat9FS) Link(cancel <-chan struct{}, input *gofuse.LinkIn, name string, out *gofuse.EntryOut) (status gofuse.Status) {
	...
	// The stat above already knows whether this inode is an extent object, so
	// re-stamp it: SetIdentity can drop a routing the inode only had from a
	// create, and an extent file whose routing is lost falls back to the classic
	// data plane (a cached negative probe would even make the server refuse it).
	if stat != nil && stat.ExtentIno != 0 {
		fs.inodes.SetExtentIno(input.Oldnodeid, stat.ExtentIno)
	}
	fs.extentMissForget(dstP)
...
func (fs *Dat9FS) renamePendingNewCommit(ctx context.Context, input *gofuse.RenameIn, oldP, newP string) (pendingRenameResult, error) {
	...
	if fs.commitQueue != nil {
		...
		entry := &CommitEntry{
			...
			PendingIndexGen: preparedMeta.Generation,
			BaseRev:         preparedMeta.BaseRev,
			Size:            preparedMeta.Size,
			Kind:            preparedMeta.Kind,
			ShadowSpill:     preparedMeta.ShadowSpill,
			Mode:            preparedMeta.Mode,
			HasMode:         preparedMeta.HasMode,
		}
		if fs.shadowStore != nil {
			entry.ShadowGen = fs.shadowStore.ActiveGeneration(newP)
		}
		fs.bindCommitEntryToPreparedPendingMeta(entry, preparedMeta)
...
func (fs *Dat9FS) Rename(cancel <-chan struct{}, input *gofuse.RenameIn, oldName string, newName string) (status gofuse.Status) {
	...
	// instead of swapping the two. The kernel forwards them on any mount that
	// negotiated RENAME2, so refuse them here; callers that probe with
	// RENAME_NOREPLACE (GNU mv, some rename fallbacks) expect EINVAL and retry
	// without flags.
	if input.Flags != 0 {
...
func (fs *Dat9FS) OpenDir(cancel <-chan struct{}, input *gofuse.OpenIn, out *gofuse.OpenOut) (status gofuse.Status) {
	...
	if dh.extentFh != 0 {
		// JuiceFS OpenDir sets CACHE_DIR only with --readdir-cache (off), and
		// a directory that has a JuiceFS handle can gain extent children that
		// the kernel's cached listing does not know about.
		//
		// The test is the handle, not "this mount has an extent profile": a
		// directory with no mirrored jfs tree lists nothing from the extent
		// data plane, so dropping KEEP_CACHE for it only made every directory
		// on an extent mount re-read on each opendir.
		out.OpenFlags = 0
	} else {
...
func (fs *Dat9FS) loadDirHandleEntries(ctx context.Context, dh *DirHandle, refresh bool) ([]DirEntry, uint64, error) {
	...
	dh.entriesGeneration = generation
	loaded := cloneLoadedDirEntries(entries)
	dh.mu.Unlock()
...
func (fs *Dat9FS) listDir(ctx context.Context, dirPath string) ([]DirEntry, error) {
	...
	generation := fs.mountViewGeneration.Load()
	cached, err := fs.loadForegroundRemoteDirectory(ctx, dirPath, generation)
	if err != nil {
		return nil, err
	}
...
func (fs *Dat9FS) mergeLayerNamespaceEntries(dirPath string, entries []DirEntry) []DirEntry {
	if fs == nil || !fs.layerEnabled() {
		return entries
	}
	fs.layerMu.RLock()
	if len(fs.layerWhiteouts) == 0 && len(fs.layerFiles) == 0 && len(fs.layerDirs) == 0 && len(fs.layerSymlinks) == 0 {
		fs.layerMu.RUnlock()
		return entries
	}
	whiteouts := make(map[string]struct{})
	for p := range fs.layerWhiteouts {
		if parentDir(p) == dirPath {
			whiteouts[path.Base(p)] = struct{}{}
		}
	}
	files := make(map[string]uint32)
	for p, mode := range fs.layerFiles {
		if parentDir(p) == dirPath {
			files[path.Base(p)] = mode
		}
	}
	dirs := make(map[string]uint32)
	for p, mode := range fs.layerDirs {
		if parentDir(p) == dirPath {
	...
	if len(files) > 0 {
		for i := range entries {
			...
			mode, ok := files[entries[i].Name]
			if !ok {
				continue
			}
			mode &= 0o777
			entries[i].Mode = dirEntryMode(false, true, mode)
			entries[i].AttrMode = mode
			entries[i].HasMode = true
			if entries[i].Ino != 0 {
				fs.inodes.UpdateMode(entries[i].Ino, mode)
			}
		}
	}
	...
	for name, mode := range dirs {
		...
		ino := fs.inodes.EnsureInode(childP, true, 0, time.Now())
		fs.inodes.UpdateMode(ino, mode)
		entries = append(entries, DirEntry{
			Name:        name,
			Ino:         ino,
			Mode:        dirEntryMode(true, true, mode),
			Mtime:       time.Now(),
			AttrMode:    mode,
			HasMode:     true,
			IsDir:       true,
	...
	for _, item := range items {
		...
		if existing, ok := fs.inodes.GetInode(childP); ok {
			if e, ok := fs.inodes.GetEntry(existing); ok {
				...
				//
				// Size: keep the cached value unless the listing proves a
				// different object (both resource ids known and unequal, or a
				// kind change). A listing that says 0 right after a JuiceFS
				// write is the case this exists for, and it is self-correcting
				// because the next authoritative stat overwrites it.
				provesDifferent := item.IsDir != e.IsDir ||
					(item.ResourceID != "" && e.ResourceID != "" && item.ResourceID != e.ResourceID)
				if !provesDifferent {
					prevSize = e.Size
				}
				// Routing: lend the cached JuiceFS inode only when the cache
				// provably describes this same object. An entry with no
				// resource id (a directory, or one built from a layer/pending
				// merge) proves nothing, and inheriting a stale ExtentIno is
				// unrecoverable: every later Open routes to the replaced
				// file's drained JuiceFS inode and returns ENOENT with no
				// classic fallback.
				//
				// Clearing is the safe direction even when it was not needed.
				// For a directory child it always clears, because directory
				// entries never store a resource id while the listing always
				// carries one: the next mutation inside that directory resolves
				// its parent through the extentEnsureParent walk instead of the
				// kernel-parent fast path, and the walk re-binds the routing.
				// Skipping the clear for directories would re-open the stale
				// case, where a directory replaced by another (or by a file)
				// keeps a routing that no longer belongs to it.
				provesSame := item.ResourceID != "" && item.ResourceID == e.ResourceID && item.IsDir == e.IsDir
...
func (fs *Dat9FS) Open(cancel <-chan struct{}, input *gofuse.OpenIn, out *gofuse.OpenOut) (status gofuse.Status) {
	...
	if ino, ok := fs.extentOpenIno(ctx, input.NodeId, p); ok {
...
func (fs *Dat9FS) Write(cancel <-chan struct{}, input *gofuse.WriteIn, data []byte) (written uint32, status gofuse.Status) {
	...
	if fh.isExtent() {
		source = "extent"
		st := fs.extentWrite(fs.jfsCtx(input.Pid, input.Uid, input.Gid), fh, input.Offset, data)
		if st != gofuse.OK {
			return 0, st
		}
...
func (fs *Dat9FS) clearCommittedAppendSnapshot(entry *CommitEntry) {
	if fs == nil || fs.openHandles == nil || entry == nil || entry.SnapshotID == "" {
		return
	}
	for _, fh := range fs.openHandles.SnapshotPath(entry.Path) {
		if fh == nil || !fh.TryLock() {
		...
		if fh.appendSnapshot && fh.DirtySeq == entry.MutationSeq && fh.ContentSnapshotID == entry.SnapshotID {
			fh.appendSnapshot = false
		}
		fh.Unlock()
	}
