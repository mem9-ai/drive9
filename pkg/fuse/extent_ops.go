...
// extentOpenIno picks the JuiceFS inode an Open should go through the extent
// data plane with, or false to leave the file on the classic path.
//
// Discovery is layout-based, never name-based: the projection is the only
// authority on which data plane a file lives on, and the profile glob only
// decides where *new* files are created. Probing jfs by name here used to let
// an orphan jfs edge capture a single-layout file that merely matched the glob
// (P1-4).
func (fs *Dat9FS) extentOpenIno(ctx context.Context, nodeID uint64, p string) (uint64, bool) {
	if ino, ok := fs.cachedExtentIno(nodeID); ok {
		return ino, true
	}
	return fs.existingExtentIno(ctx, p)
}

// extentMissTTL bounds how long "this path is not an extent file" is trusted.
// It only has to outlive the burst of syscalls one workload level issues
// against the same file; the worst a stale miss can do is route a file another
// mount turned into an extent file through the classic path until the kernel's
// attribute TTL expires and the next lookup re-probes.
const extentMissTTL = 2 * time.Second

// extentMissCached reports a recently observed non-extent path.
func (fs *Dat9FS) extentMissCached(p string) bool {
	fs.extentMissesMu.Lock()
	defer fs.extentMissesMu.Unlock()
	at, ok := fs.extentMisses[p]
	if !ok {
		return false
	}
	if time.Since(at) > extentMissTTL {
		delete(fs.extentMisses, p)
		return false
	}
	return true
}

// extentMissCap bounds the negative cache. It is a 2s hint, so throwing it away
// wholesale once the cap is crossed costs at most one re-probe per path; the
// alternative (pruning only expired entries) leaves the map unbounded while a
// workload walks a tree of glob-matching classic files, and every insert then
// rescans the whole map under a mutex the extent Open/Unlink/Rename/SetAttr
// paths share.
const extentMissCap = 4096

func (fs *Dat9FS) extentMissRemember(p string) {
	fs.extentMissesMu.Lock()
	defer fs.extentMissesMu.Unlock()
	if fs.extentMisses == nil {
		fs.extentMisses = make(map[string]time.Time)
	}
	if len(fs.extentMisses) >= extentMissCap {
		now := time.Now()
		for k, at := range fs.extentMisses {
			if now.Sub(at) > extentMissTTL {
				delete(fs.extentMisses, k)
			}
		}
		if len(fs.extentMisses) >= extentMissCap {
			// Still over: drop the hint rather than grow without limit.
			fs.extentMisses = make(map[string]time.Time, extentMissCap/2)
		}
	}
	fs.extentMisses[p] = time.Now()
}

// extentMissForget drops the cached miss for a path this mount just changed.
func (fs *Dat9FS) extentMissForget(p string) {
	if p == "" {
		return
	}
	fs.extentMissesMu.Lock()
	delete(fs.extentMisses, p)
	fs.extentMissesMu.Unlock()
}
...
// only runs on mounts that opted into the extent data plane, and a negative
// answer is remembered briefly: otherwise every Open/Unlink/Rename/SetAttr of
// a classic file that merely matches the glob spends a full round trip.
func (fs *Dat9FS) existingExtentIno(ctx context.Context, p string) (uint64, bool) {
	...
	if fs.extentMissCached(p) {
		return 0, false
	}
	if ino, ok := fs.extentStatIno(ctx, p); ok {
		fs.extentMissForget(p)
		return ino, true
	}
	fs.extentMissRemember(p)
	return 0, false
}
...
func (fs *Dat9FS) extentSetAttr(cancel <-chan struct{}, input *gofuse.SetAttrIn, entry *InodeEntry, out *gofuse.AttrOut) gofuse.Status {
	...
	if input.Valid&gofuse.FATTR_SIZE != 0 {
		// See sqliteWALIndexMinSize: the only place allowed to report a size
...
func (fs *Dat9FS) extentRename(cancel <-chan struct{}, input *gofuse.RenameIn, oldP, newP, oldName, newName string) gofuse.Status {
	...
	// Both names changed identity (or became one), so a cached "not an extent
	// file" answer for either is no longer evidence about the path.
	fs.extentMissForget(oldP)
	fs.extentMissForget(newP)
	if replacedIno != 0 {
