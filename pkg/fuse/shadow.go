...
func (s *ShadowStore) writeFullIfGeneration(remotePath string, data []byte, baseRev int64, expectedGen uint64, tx *layerRestoreTxn, prepareMeta layerRestoreMetaPrepare) (bool, error) {
	...
	return s.installLayerRestoreTempIfGeneration(remotePath, tmpPath, int64(len(data)), baseRev, expectedGen, tx, prepareMeta)
}
...
func (s *ShadowStore) installLayerRestoreTempIfGeneration(remotePath, tmpPath string, size, baseRev int64, expectedGen uint64, tx *layerRestoreTxn, prepareMeta layerRestoreMetaPrepare) (bool, error) {
	pl := s.acquirePathLock(remotePath)
	defer s.releasePathLock(remotePath, pl)
	...
	rollback := func(primary error) (bool, error) {
		result := rollbackLayerRestoreFailure(tx, primary)
		s.mu.Unlock()
		return false, result
	}
	...
	publishMeta, err := prepareMeta(size)
	if err != nil {
		_ = newFd.Close()
		return rollback(err)
	}
	// Marker removal is the durable commit point. Do it before publishing the
	// new shadow or metadata to process memory. If marker cleanup or its
	// directory fsync fails, rollback still runs while both path locks and s.mu
	// exclude successor writers, so the process and the next restart agree on
	// the old pair.
...
// shadowRemoveCleanup captures the disk cleanup work remaining after the
// per-path maps have been updated under s.mu. Callers perform these filesystem
// operations after releasing the lock to avoid holding s.mu over slow I/O.
type shadowRemoveCleanup struct {
	// removedSize is the shadow size to subtract from pendingBytes (after unlock).
	removedSize int64

	// For the retire path: rename srcDiskPath → retiredPath so Has() and
	// ensureShadowFile don't see the stale data while pinned readers keep
	// using the retired fd. If rename fails, fall back to os.Remove(srcDiskPath).
	retire      bool
	srcDiskPath string
	retiredPath string
...
// removeCoreLocked performs all in-memory map removal for remotePath under
// s.mu and returns the disk cleanup to run after unlocking. If expectedGen is
// nonzero, the removal is skipped (returns false) when the current writeGen
...
func (s *ShadowStore) removeCoreLocked(remotePath string, expectedGen uint64, reason shadowRetirementReason) (shadowRemoveCleanup, bool) {
	...
	cleanup.immediatePath = diskPath
	return cleanup, true
}

// runRemoveCleanup performs the disk cleanup returned by removeCoreLocked and
// accounts the pendingBytes delta. Must be called after releasing s.mu.
func (s *ShadowStore) runRemoveCleanup(cleanup shadowRemoveCleanup) {
	if cleanup.removedSize > 0 {
		s.pendingBytes.Add(-cleanup.removedSize)
	}
	if cleanup.retire {
		...
		_ = os.Remove(cleanup.immediatePath)
	}
}
...
// hold so a concurrent same-path WriteFull/WriteAt/Ensure cannot bump writeGen
...
func (s *ShadowStore) renameIfGenerations(oldPath, newPath string, expectedOldGen, expectedNewGen uint64, tx *layerRestoreTxn, prepareMeta layerRestoreMetaPrepare) (moved, matched bool, err error) {
	pla, plb := s.acquireTwoPathLocks(oldPath, newPath)
	defer s.releaseTwoPathLocks(oldPath, newPath, pla, plb)
	...
	if oldPath == newPath {
		...
		publishMeta, err := prepareMeta(size)
		if err != nil {
			result := rollbackLayerRestoreFailure(tx, err)
			s.mu.Unlock()
	...
	publishMeta, err := prepareMeta(sf.size)
	if err != nil {
		return rollback(err)
	}
	if err := tx.commit(); err != nil {
