...
// MaxTiDBCloudSpendingLimit is the tenant-facing quota API maximum. In the
...
func (s *Store) CountTiDBCloudFreeTenants(ctx context.Context, organizationID string) (count int, err error) {
	...
	err = s.db.QueryRowContext(ctx, `SELECT COALESCE(SUM(free_count), 0)
		...
		organizationID, TenantPoolBindingFree, tidbCloudNativeProvider, TenantDeleted).Scan(&count)
	return count, err
}
...
func (s *Store) HasTenantPoolOwnership(ctx context.Context, tenantID string) (owned bool, err error) {
	...
	err = s.db.QueryRowContext(ctx, `SELECT EXISTS (
		...
		WHERE b.tenant_id = ? AND TRIM(b.pool_id) <> '')`, tenantID).Scan(&owned)
	if err != nil {
...
func (s *Store) ObservePendingMutations(ctx context.Context) ([]MutationBacklogObservation, error) {
	...
	rows, err := s.db.QueryContext(ctx, observePendingMutationsSQL)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
