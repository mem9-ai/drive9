...
func ProcessCreationTime(pid int) (uint64, error) {
	...
	if commEnd < 0 || commEnd+2 >= len(stat) {
		return 0, fmt.Errorf("inspect pid %d: malformed /proc stat", pid)
	}
	fields := strings.Fields(stat[commEnd+2:])
	if len(fields) <= 19 {
		return 0, fmt.Errorf("inspect pid %d: short /proc stat", pid)
	}
	startTime, err := strconv.ParseUint(fields[19], 10, 64)
	if err != nil {
		return 0, fmt.Errorf("inspect pid %d start time: %w", pid, err)
	}
