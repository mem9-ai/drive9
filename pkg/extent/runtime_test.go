...
func TestApplyChunkCacheDirJfsSubdir(t *testing.T) {
	conf := chunk.Config{BufferSize: 32 << 20}
	...
	if conf.BufferSize != juiceChunkBufferSize {
		t.Fatalf("BufferSize=%d, want JuiceFS --buffer-size %d", conf.BufferSize, juiceChunkBufferSize)
	}
...
func TestApplyChunkCacheDirEmptyIsNoop(t *testing.T) {
	conf := chunk.Config{CacheDir: "memory", Prefetch: 7}
	applyChunkCacheDir(&conf, "")
	if conf.CacheDir != "memory" || conf.Prefetch != 7 {
...
func TestApplyChunkCacheDirKeepsDiskCacheWithoutStaging(t *testing.T) {
	conf := chunk.Config{
		BlockSize:      4 << 20,
		CacheSize:      1 << 30,
		CacheDir:       "memory",
		...
		BufferSize:     32 << 20,
		CacheFullBlock: true,
		// NewRuntime pins chunk-store staging off, so SelfCheck must not turn
		// it back on: Flush/Fsync may not return before the object PUT.
		Writeback: false,
	}
	applyChunkCacheDir(&conf, t.TempDir())
	conf.SelfCheck("drive9-extent")
	if conf.Writeback {
		t.Fatal("SelfCheck turned chunk-store staging back on")
	}
