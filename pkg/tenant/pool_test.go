...
func TestPoolAutoEmbeddingProfileForTenantBackfillsConfiguredDefaultProfile(t *testing.T) {
	...
	pool := NewPool(PoolConfig{
		...
		TiDBAutoEmbeddingAPIKey:  "sk-backfill",
		TiDBAutoEmbeddingAPIBase: "https://example.openai.azure.com",
	}, enc)
	defer pool.Close()
	pool.SetMetaStore(metaStore)

	got, err := pool.autoEmbeddingProfileForTenant(context.Background(), &meta.Tenant{ID: "tenant-backfill"})
	if err != nil {
		t.Fatalf("autoEmbeddingProfileForTenant: %v", err)
	}
	if got.schemaProfile.Model != "openai/text-embedding-3-small" || got.schemaProfile.Dimensions != 1536 {
		t.Fatalf("schema profile = %+v", got.schemaProfile)
	}
	if got.schemaProfile.OptionsJSON != `{"dimensions":1536}` {
		t.Fatalf("options_json = %q", got.schemaProfile.OptionsJSON)
	}
	if got.provider.APIKey != "sk-backfill" || got.provider.APIBase != "https://example.openai.azure.com" {
		t.Fatalf("provider profile = %+v", got.provider)
	}

	stored, err := metaStore.GetTenantAutoEmbeddingProfile(context.Background(), "tenant-backfill")
	if err != nil {
	...
	if string(stored.APIKeyCipher) == "sk-backfill" {
