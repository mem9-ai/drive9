...
//
// Quota for extent files is enforced by datastore.ExtentQuotaLimit inside the
// meta transaction, not by JuiceFS's directory counters: the jfs_dir_stats
// table and the usedSpace counter are not maintained here, so anything reading
// them would see zeros. They are deliberately absent rather than shipped
// unmaintained, because a table that looks like enforcement invites a later
// reader to assume there is some.
