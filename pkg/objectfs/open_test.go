...
func TestConnectionString(t *testing.T) {
	...
	tests := []struct {
		...
		{"cos://b/k?region=ap-beijing", `:s3,provider=TencentCOS,env_auth=true,directory_markers=true,region=ap-beijing,endpoint="https://cos.ap-beijing.myqcloud.com":b/k`},
		{"oss://b/k", ":s3,provider=Alibaba,env_auth=true,directory_markers=true:b/k"},
		{"tos://b/k?region=cn-beijing", `:s3,provider=Other,env_auth=true,directory_markers=true,region=cn-beijing,endpoint="https://tos-s3-cn-beijing.volces.com",force_path_style=false:b/k`},
