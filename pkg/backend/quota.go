...
// tasks (any status). Returns true when the count exceeds the configured limit.
func (b *Dat9Backend) videoLLMQuotaExceededTx(tx *sql.Tx) bool {
	if b.maxVideoLLMFiles <= 0 {
		return false
	}
	var count int64
	if err := tx.QueryRow(`SELECT COUNT(DISTINCT resource_id) FROM semantic_tasks WHERE task_type = 'video_extract_visual'`).Scan(&count); err != nil {
