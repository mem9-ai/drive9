...
type Part struct {
	...
	ChecksumSHA256 string // base64-encoded SHA-256
	ChecksumCRC32C string // base64-encoded CRC32C
}
...
type UploadPartURL struct {
	...
	ChecksumSHA256 string            `json:"checksum_sha256,omitempty"` // expected SHA-256 checksum
	ChecksumCRC32C string            `json:"checksum_crc32c,omitempty"` // expected CRC32C checksum
...
type S3Client interface {
	// CreateMultipartUpload initiates a new multipart upload with the given
	// checksum algorithm. Use ChecksumAlgoSHA256 or ChecksumAlgoCRC32C.
	...
	// At most one of checksumSHA256/checksumCRC32C may be non-empty.
	PresignUploadPart(ctx context.Context, key, uploadID string, partNumber int, partSize int64, checksumSHA256 string, checksumCRC32C string, ttl time.Duration) (*UploadPartURL, error)
